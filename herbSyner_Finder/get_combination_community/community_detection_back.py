...
def get_disease_communities(G,communities,DI,DH,disease_name):
...
def get_disease_communities_all(G,communities):
    node_list = list(G.nodes())
...
def filter_admet(DHC,DIC,ADMET):
...
def merge_cluster_files(SZfile, SHfile, HLfile, JWfile, SGfile):
    JWfile['FJ'] = 'JWDCT'
    SGfile['FJ'] = 'SGMHT'
    SHfile['FJ'] = 'SHZKJN'
    SZfile['FJ'] = 'SZJQT'
...
def data_processing_before_sankey_diagram(sum_ADMET_file):
    sankey_diagram_file= sum_ADMET_file.iloc[:, [0,2,4,5,13]].drop_duplicates()
    sankey_diagram_file.columns = ['disease', 'herb', 'ingredient','distance','Prescription']
...
def main1():
    ...
    HHf='result/SH_herb_herb_distance.csv'
    IIf='result/SH_ingre_ingre_distance.csv'
    DHf='result/SH_herb_disease_pd.csv'
    DIf='result/SH_herb_disease_ingre_pd.csv'
    ...
    SHresult,SHsubset_data_I,SHsubset_data_H = get_disease_communities(SH_G,SH_communities,SH_DI,SH_DH, disease_name )
    SHDHC_select,SHDIC_ADMET= filter_admet(SHsubset_data_H,SHsubset_data_I,ADMET)
    SH_node_detect_df=get_disease_communities_all(SH_G,SH_communities)

##Process the results of five formulations for the next steps.

##3 data_processing_before_sankey_diagram
...
# sankey_diagram_file.to_csv('result/sankey_diagram_file.csv', index=None)

##4 deal_cluster_result_for_hierarchical_cluster
...
# matrix_file,min_value,type_file=matrix_file_for_hierarchical_cluster(matrix_sum_file,matrix_pre_file)
# matrix_herb_type_file=sankey_diagram_file.iloc[:,[1,2]]

# matrix_file.to_csv('result/matrix_cluster_result_admet.csv')
# type_file.to_csv('result/matrix_type_result_admet.csv', index=None)
# matrix_herb_type_file.to_csv('result/matrix_herb_result_admet.csv', index=None)
