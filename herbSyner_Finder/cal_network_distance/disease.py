...
class Disease:
    ...
    def read_data(self):
        ...
        data = data.dropna(subset = ['Gene ID'])
        data['Gene ID'] = data['Gene ID'].astype(int)
    ...
    def cal_disease_herb_ingre_z_score(self, disease, herb, distance_method, herb_ingre_dict, ingre_tar_dict, random_time, seed):
        ...
        print('there are {} ingredient in herb {}'.format(len(ingres), herb))
        ingre_z_score_dict = {}
        for ingre in tqdm.tqdm(ingres):
